...
export default function CryptoTable() {
  ...
  return (
    <div className=" p-4 rounded-lg w-full max-w-sm text-white">
      {/* Search */}
      <div className="flex items-center bg-transparent px-3 py-2 rounded-2xl mb-4 border-2 border-gray-800">
        ...
        <input
          ...
          className="bg-transparent outline-none text-sm placeholder-white flex-1"
        />
      </div>
      ...
      {/* Table Header */}
      <div className="flex justify-between text-[11px]  px-2 mb-2">
        <span>Coin</span>
      ...
      <div className="max-h-48 overflow-y-auto scrollbar-thin scrollbar-thumb-[#848e9c]/30 scrollbar-track-transparent">
        {filteredCoins.map((coin, index) => (
          <div
            key={index}
            className="flex justify-between items-center text-sm py-2 px-2"
          >
            {/* Coin */}
            ...
            {/* 24h Change */}
            <span
              className={`text-xs ${coin.change >= 0 ? "text-[#0ecb81]" : "text-[#f6465d]"
                }`}
            >
